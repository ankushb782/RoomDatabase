...
public class AdapterRecyclerView extends RecyclerView.Adapter<AdapterRecyclerView.ViewHolder> {
    ...
    public AdapterRecyclerView(ArrayList<ModelData> modelData, Context ctx){
        /**
         * Inisiasi data dan variabel yang akan digunakan
         */
        daftarModelData = modelData;
        context = ctx;
    ...
    public void onBindViewHolder(ViewHolder holder, final int position) {
         ...
         */
        final String name = daftarModelData.get(position).getName();
        ...
        holder.cvMain.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                 ...
                 */
                ModelData modelData = db.dataDAO().selectBarangDetail(daftarModelData.get(position).getId());
                context.startActivity(RoomReadSingleActivity.getActIntent((Activity) context).putExtra("data", modelData));
            }
        });
    ...
    private void onDeteleBarang(int position){
        db.dataDAO().deleteBarang(daftarModelData.get(position));
        daftarModelData.remove(position);
        notifyItemRemoved(position);
        notifyItemRangeRemoved(position, daftarModelData.size());
    }

    private void onEditBarang(int position){
        context.startActivity(RoomCreateActivity.getActIntent((Activity) context).putExtra("data", daftarModelData.get(position)));
    }

    @Override
    public int getItemCount() {

        return daftarModelData.size();
    }
}
